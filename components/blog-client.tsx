        ...
        <section className="mb-8 md:mb-12">
          ...
          {hasActiveFilters && (
            <div className="mb-4 p-3 md:p-4 bg-secondary/30 rounded-lg flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
              <div className="flex flex-wrap gap-2">
                {filters.categories.filter(c => c !== "All").map(cat => (
                  <Badge key={cat} variant="secondary" className="text-xs">
                    {cat}
                    <button onClick={() => handleCategoryToggle(cat)} className="ml-1 hover:text-destructive">
                      <X className="w-3 h-3" />
                    </button>
                  </Badge>
                ))}
              ...
              <Button variant="ghost" size="sm" onClick={clearAllFilters} className="text-xs whitespace-nowrap">
                Clear All
              </Button>
            </div>
          )}
