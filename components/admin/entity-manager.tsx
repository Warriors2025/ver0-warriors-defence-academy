...
import { Plus, Pencil, Trash2, Save, X, CheckCircle, Search } from "lucide-react"
...
export type FieldDef = {
  key: string
  label: string
  type: "text" | "textarea" | "number" | "image" | "boolean" | "json" | "lines" | "password"
  required?: boolean
...
export function EntityManager({ title, description, apiPath, fields, emptyRow, seoEntity }: EntityManagerProps) {
  ...
  function fieldClass(field: FieldDef) {
    return field.type === "textarea" || field.type === "image" || field.type === "json" || field.type === "lines"
      ? "sm:col-span-2"
      : ""
  }
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          {rows.length === 0 ? (
            ...
            rows.map((row) => (
              <div key={String(row.id)} className="flex items-center justify-between gap-4 px-5 py-4 border-b border-border last:border-0">
                <div>
                  <p className="font-medium text-foreground">{String(row[displayField] ?? "Untitled")}</p>
                  {fields[1] && <p className="text-xs text-muted-foreground mt-0.5 truncate max-w-md">{String(row[fields[1].key] ?? "")}</p>}
                </div>
                <div className="flex gap-2 shrink-0">
                  {seoEntity && row[slugField] && (
